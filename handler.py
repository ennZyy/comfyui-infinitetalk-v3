...
# LoadAudio / Load Image read from this directory (same as ComfyUI UI uploads)
...
def validate_input(job_input):
    ...
    if audios is not None:
        if not isinstance(audios, list) or not all(
            "name" in item and "audio" in item for item in audios
        ):
            return (
                None,
                "'audios' must be a list of objects with 'name' and 'audio' keys",
            )
...
def _strip_base64_payload(data_uri_or_b64: str) -> str:
...
def _safe_input_basename(name: str) -> tuple[str | None, str | None]:
    """Reject path traversal; return basename only."""
    if not name or not isinstance(name, str):
        return None, "Each audio 'name' must be a non-empty string"
    base = os.path.basename(name.replace("\\", "/"))
    if not base or base != name.replace("\\", "/"):
        return (
            None,
            f"Invalid audio name (use a plain filename only): {name!r}",
        )
    if base in (".", "..") or base.startswith("."):
        return None, f"Invalid audio name: {name!r}"
    return base, None


def save_audios_to_input(audios):
    """
    Decode base64 audio payloads and write them into ComfyUI's input directory
    (same end result as uploading via the ComfyUI web UI).
    """
    if not audios:
        return {"status": "success", "message": "No audios to save", "details": []}

    responses = []
    errors = []
    os.makedirs(COMFY_INPUT_DIR, exist_ok=True)

    print(f"worker-comfyui - Saving {len(audios)} audio file(s) to {COMFY_INPUT_DIR}...")

    for item in audios:
        try:
            name, err = _safe_input_basename(item["name"])
            if err:
                errors.append(err)
                continue

            raw_b64 = _strip_base64_payload(item["audio"])
            blob = base64.b64decode(raw_b64, validate=True)
            dest = os.path.join(COMFY_INPUT_DIR, name)
            with open(dest, "wb") as f:
                f.write(blob)

            msg = f"Successfully saved {name}"
            responses.append(msg)
            print(f"worker-comfyui - {msg}")
        except base64.binascii.Error as e:
            err = f"Error decoding base64 for {item.get('name', 'unknown')}: {e}"
            print(f"worker-comfyui - {err}")
            errors.append(err)
        except OSError as e:
            err = f"Error writing {item.get('name', 'unknown')}: {e}"
            print(f"worker-comfyui - {err}")
            errors.append(err)
        except Exception as e:
            err = f"Unexpected error saving {item.get('name', 'unknown')}: {e}"
            print(f"worker-comfyui - {err}")
            errors.append(err)
    ...
    if errors:
        return {
            "status": "error",
            "message": "Some audio files failed to save",
            "details": errors,
        }
    return {
        "status": "success",
        "message": "All audio files saved successfully",
        "details": responses,
    }
...
def handler(job):
    ...
    input_audios = validated_data.get("audios")
    ...
    if input_audios:
        audio_result = save_audios_to_input(input_audios)
        if audio_result["status"] == "error":
            return {
                "error": "Failed to save one or more input audio files",
